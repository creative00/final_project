...
public class ListController0 extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//전체 게시물의 갯수를 카운트하여 반환한다.
		//게시물의 갯수를 통해 가상번호를 부여하거나 
		//전체 페이지 수를 계산한다.
		int totalCount = dao.selectCount(map);
		/* 페이징 처리 start */
		/*
		 서블릿에서 application내장객체를 얻어온 후
		 web.xml에 설정한 컨텍스트 초기화 파라미터를 읽어온다.
		 한페이지에 출력할 게시물의 갯수와 한블럭 당 
		 출력할 페이지 번호의 갯수이다.  
		 */
		...
		int blockPage =
				Integer.parseInt(
						application.getInitParameter("POSTS_PER_BLOCK"));
		/*
		목록에 첫 진입 시에는 무조건 1페이지로 가정한 후 게시물을 얻어온다. 
		특정 페이지로 진입한 경우에는 파라미터를 받아 구간을 계산한다. 
		*/
		int pageNum = 1;
		...
		//모델1 방식의 게시판에서 사용했던 메서드 그대로 사용한다.
		String pagingImg = BoardPage.pagingStr(totalCount, pageSize, 
					blockPage, pageNum, "../www.noticeboard/list.do");
		//View로 전달할 데이터를 Map에 저장한다.
		//목록 하단에 출력할 페이지 번호
		map.put("pagingImg", pagingImg);
		//전체 게시물의 갯수
		map.put("totalCount", totalCount);
		//한 페이지 당 출력할 게시물의 갯수(설정값)
		...
		req.getRequestDispatcher("/WWW/01Login/Notice.jsp").forward(req, resp);
	}
}
